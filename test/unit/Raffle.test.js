    ...
    : describe("Raffle Unit Tests", function () {
          ...
          describe("fulfillRandomWords", function () {
              ...
              it("picks a winner, resets the lottery and sends money", async function () {
                  ...
                  // performUpkeep (mocks being chainlink keepers)
                  // fulfillRandomWords (mocks being the chainlink VRF)
                  // we will have to wait for the fulfillRandomWords to be called, *while testing on testnet*
                  await new Promise(async (resolve, reject) => {
                      // Setting up the listener
                      raffle.once("WinnerPicked", async () => {
                          console.log("Found the WinnerPicked event!")
                          try {
                              const recentWinner = await raffle.getRecentWinner() // by using console.log, we established that accounts[1] is gonna be winner
                              const raffleState = await raffle.getRaffleState()
